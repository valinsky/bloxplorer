...
def test_mempool():
    mempool.get()
    mempool.make_request.assert_called_with('GET', 'mempool')


def test_mempool_txids():
    mempool.get_txids()
    mempool.make_request.assert_called_with('GET', 'mempool/txids')


def test_mempool_latest_txs():
    mempool.get_latest_txs()
    mempool.make_request.assert_called_with('GET', 'mempool/recent')
